...
pub(super) fn resolve_output_tail_resolution(
    ...
    match (run_id, task_id) {
        ...
        (Some(_), Some(_)) => Err(invalid_input_error("provide exactly one of run_id or task_id")),
        (None, None) => Err(invalid_input_error("provide exactly one of run_id or task_id")),
    }
}

fn resolve_output_tail_run_id(project_root: &str, run_id: String) -> Result<OutputTailResolution> {
    ensure_safe_run_id(run_id.as_str())?;
    let run_dir = crate::services::operations::resolve_run_dir_for_lookup(project_root, run_id.as_str())?
        .ok_or_else(|| not_found_error(format!("run directory not found for {run_id}")))?;
    Ok(OutputTailResolution { run_id, run_dir, resolved_from: "run_id" })
}

fn resolve_output_tail_task_id(project_root: &str, task_id: String) -> Result<OutputTailResolution> {
    let workflows = workflow_candidates_for_task(project_root, task_id.as_str())?;
    if workflows.is_empty() {
        return Err(not_found_error(format!("workflow not found for task {task_id}")));
    }

    for workflow in workflows {
        if let Some((run_id, run_dir)) = resolve_latest_workflow_run_dir(project_root, workflow.id.as_str())? {
            return Ok(OutputTailResolution { run_id, run_dir, resolved_from: "task_id" });
        }
    }

    Err(not_found_error(format!("run directory not found for task {task_id}")))
}

fn workflow_candidates_for_task(project_root: &str, task_id: &str) -> Result<Vec<OrchestratorWorkflow>> {
...
fn compare_workflow_candidates(left: &OrchestratorWorkflow, right: &OrchestratorWorkflow) -> Ordering {
...
fn resolve_latest_workflow_run_dir(project_root: &str, workflow_id: &str) -> Result<Option<(String, PathBuf)>> {
    ...
    for run_id in run_ids {
        let Some(run_dir) = crate::services::operations::resolve_run_dir_for_lookup(project_root, run_id.as_str())?
        else {
            continue;
        };
    ...
    candidates.sort_by(|left, right| {
        right.2.cmp(&left.2).then_with(|| right.3.cmp(&left.3)).then_with(|| left.0.cmp(&right.0))
    });

    Ok(candidates.into_iter().next().map(|(run_id, run_dir, _, _)| (run_id, run_dir)))
}
