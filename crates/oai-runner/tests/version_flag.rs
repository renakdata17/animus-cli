...
fn version_output_contains_git_hash_in_parens() {
    ...
    assert!(
        !hash.is_empty(),
        "git hash between parentheses should not be empty, got: {trimmed}"
    );

    let valid = hash == "unknown" || hash.chars().all(|c| c.is_ascii_hexdigit());
    assert!(
        valid,
        "git hash should be hex digits or 'unknown', got: {hash}"
    );
}
...
fn version_output_format_matches_expected_pattern() {
    ...
    assert!(
        trimmed.ends_with(')'),
        "version output should end with ')', got: {trimmed}"
    );
}
