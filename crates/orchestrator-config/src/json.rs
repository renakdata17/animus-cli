...
pub fn write_json_pretty<T: Serialize>(path: &Path, value: &T) -> Result<()> {
    ...
    let file_name = path.file_name().and_then(|value| value.to_str()).unwrap_or("state.json");
        ...
        Err(original_error) => {
            if path.exists() {
                ...
                std::fs::rename(&tmp_path, path).with_context(|| {
                    format!("failed to atomically move temp file {} to {}", tmp_path.display(), path.display())
                })?;
                Ok(())
            } else {
