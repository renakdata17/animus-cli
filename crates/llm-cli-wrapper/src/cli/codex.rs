...
#[async_trait]
impl CliInterface for CodexCli {
    ...
    async fn check_auth(&self) -> Result<bool> {
        // Codex stores auth in config, just check if it's available
    ...
    async fn get_version(&self) -> Result<String> {
        ...
        if output.is_success() {
            ...
            Err(Error::ExecutionFailed("Failed to get Codex version".to_string()))
        }
    }

    async fn execute(&self, command: &CliCommand) -> Result<CliOutput> {
        debug!("Executing Codex command: {}", command.prompt);

        let mut args = vec!["exec".to_string(), "--skip-git-repo-check".to_string()];

        if let Some(model) = command.model_for_cli("codex") {
